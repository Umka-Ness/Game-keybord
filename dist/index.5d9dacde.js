...
})({"hhQrA":[function(require,module,exports) {
...
let textList = "The Dursleys had everything they wanted, but they also had a secret, and their greatest fear was that somebody would discover it. They didn’t think they could bear it if anyone found out about the Potters. Mrs. Potter was Mrs. Dursley’s sister, but they hadn’t met for several years; in fact, Mrs. Dursley pretended she didn’t have a sister, because her sister and her good-for-nothing husband were as unDursleyish as it was possible to be. The Dursleys shuddered to think what the neighbors would say if the Potters arrived in the street. The Dursleys knew that the Potters had a small son, too, but they had never even seen him. This boy was another good reason for keeping the Potters away; they didn’t want Dudley mixing with a child like that.";
let devList = "The Dursleys had everything they wanted, but they also had a secret, and their greatest fear was that somebody would discover it. They didn’t think they could bear it if anyone found out about the Potters. Mrs. Potter was Mrs. Dursley’s sister, but they hadn’t met for several years; in fact, Mrs. Dursley pretended she didn’t have a sister, because her sister and her good-for-nothing husband were as unDursleyish as it was possible to be. The Dursleys shuddered to think what the neighbors would say if the Potters arrived in the street. The Dursleys knew that the Potters had a small son, too, but they had never even seen him. This boy was another good reason for keeping the Potters away; they didn’t want Dudley mixing with a child like that.";
let error = 0;
let speed = 0;
let oldSpeed = 0;
let allWords = 0;
...
document.addEventListener("click", (e)=>{
    if (e.target === closeIcoError || e.target === backdrop || e.target === useModal) modalError.classList.add("is-hidden");
});
document.body.addEventListener("keydown", (e)=>{
    if (btn.disabled === true) {
        pushError(e.key);
        if (e.key === "CapsLock" || e.key === "Shift" || e.key === "Alt" || e.key === "Control" || e.key === "AltGraph" || e.key === "Tab" || e.key != textList[0]) return;
        else {
            e.preventDefault;
            indexPlus(e.key);
            checText(e.key);
...
btn.addEventListener("click", (event)=>{
    ...
    const checkList = setInterval(()=>{
        if (textList.length === 0) {
            ...
            if (sec >= textList.length / 3) console.log(textList.length / 3);
            else if (sec <= textList.length / 3.5 && sec >= textList.length / 4) ;
            else textList.length;
            return;
        }
    }, 250);
});
function addErrorsInModalErrors() {
    ...
    for (const i of wordsError.error)falseWords.textContent += i;
}
// delete old text
function indexPlus(key) {
    ...
    if (key === textList[0]) {
        ...
        if (key === " " || textList.length === 0) {
            wordsError.data = wordsError.data.trim();
            findErrors();
        } else wordsError.comparativeData += key;
    }
}
...
function pushError(key) {
    if (key === "CapsLock" || key === "Shift" || key === "Alt" || key === "Control" || key === "AltGraph" || key === "Tab" || key === textList[0]) return;
    else {
        error += 1;
        speed -= 1;
        console.log(error);
...
function speedTest() {
    const timeout = setInterval(()=>{
        if (oldSpeed === 0) {
            ...
            if (localStorage.getItem(KEY) != coins.textContent) localStorage.setItem(KEY, speed);
            return;
        } else if (oldSpeed <= 2) oldSpeed = 0;
        else if (oldSpeed === -1) oldSpeed = 0;
        else {
            oldSpeed = 0;
            speed += 1;
            coins.textContent = speed;
...
function counterDoneWords() {
    ...
    if (countDone <= 0) countProcentDone.textContent = "0 %";
    else {
...
},{}]},["hhQrA","1Z4Rq"], "1Z4Rq", "parcelRequire18c4")
